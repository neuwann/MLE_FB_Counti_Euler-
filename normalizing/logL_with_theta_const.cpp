...
constexpr int cutcut=int_cut*int_cut*int_cut;
constexpr int plot_size=int_m+1;
...
int plot(double p,double q,double h,Vec theta,Vec gama,Mat O,Mat A,Vec B,double bound){
    Mesh M;
    Vec T;
    Eigen::MatrixXd P=MatrixXd::Zero(cutcut,plot_size);
    double interval=bound/cut;
    std::ofstream file("logL.csv");
    for(int k=0;k<int_cut;k++){
        for(int l=0;l<int_cut;l++){
            for(int i=0;i<int_cut;i++){
                T(0)=interval*k;
                T(1)=interval*l;
                T(2)=interval*i;
                M(k,l)=logL(p,q,h,T,gama,O,A,B);
                P(k*int_cut*int_cut+int_cut*l+i,0)=T(0);
                P(k*int_cut*int_cut+int_cut*l+i,1)=T(1);
                P(k*int_cut*int_cut+int_cut*l+i,2)=T(2);
                P(k*int_cut*int_cut+int_cut*l+i,3)=M(k,l);
            }
        }
    }
    file << P << "\n";
...
int main(){
    ...
    //printf("logc=%f\n",log(std::real(c(p,q,h,theta_ex,gama_ex))));
    //printf("logL=%f\n",logL(p,q,h,theta_ex,gama_ex,O,A,B));
    return 0;
}